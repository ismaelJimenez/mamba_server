...
def generate_component_configuration(component_name="",
    ...
    The configuration dictionary is composed of the local configuration, then the configurations from file
    that are not in local, and then the settings that are neither in file nor in local but have default values.

    Args:
        component_name (str, optional): The component name. Only used for feedback on the error handling.
        settings (dict, optional): The dictionary with the description of the component settings.
        config_file (dict, optional): The dictionary of the component settings retrieved from component.config.json.
        local_config (dict, optional): The dictionary of the component settings passed via launch file.

    Returns:
        dict: The validated component configuration parameters.

    Raises:
        ComponentConfigException: If a required setting is not present in local nor in file parameters.
        ComponentSettingsException: If a setting is missing the 'required' field or the 'default' field if it is not a required parameter.
