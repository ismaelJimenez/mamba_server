...
class ThreadedTriggerInHandler:
    def __init__(self, tm_id, _inst, rx, _shared_memory, shared_memory_setter, arg):
        ...
        try:
            print("DENTRO TRIGGER 1")
            _inst.write('*OPC?')  # This waits for trigger
            _shared_memory[shared_memory_setter[
                tm_id]] = 1
            print("DENTRO TRIGGER 2")
        except:
            _shared_memory[shared_memory_setter[
                tm_id]] = 'timedout'
...
class SpectrumAnalyzer(VisaControllerBase):
    ...
    def __init__(self, context, local_config=None):
        super(SpectrumAnalyzer, self).__init__(os.path.dirname(__file__), context,
                                        local_config)
    ...
    def _service_preprocessing(self, service_request: IoServiceRequest,
        ...
        if service_request.id == 'SA_TC_QUERY_TRIGGER_IN':
            ...
            self._trigger_in_thread = threading.Thread(target=ThreadedTriggerInHandler, args=(
                service_request.id,
                self._inst,
                self._context.rx,
                self._shared_memory,
